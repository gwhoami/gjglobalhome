...
import getDay from '../../util/Formats/getDay';
...
export const InputText = React.memo(
  ({
    ...
    const isNotValid = () =>
      required && formRef.current.isSubmit && !formRef.current[formKey];
    ...
    return (
      <div className={`${styleClass}${isNotValid() ? ' mark-err' : ''}`}>
        <label className={`text-gray-600 ${required ? ' required' : ''}`}>
          {label}
        </label>
        <input
          type="text"
        ...
        />
        {isNotValid() && (
          <div className="flex justify-start items-center text-red-500 text-xs mt-1">
            {required}
          </div>
        )}
      </div>
...
export const InputEmail = React.memo(
  ({
    ...
    const isNotValid = () =>
      required &&
      formRef.current.isSubmit &&
      (!formRef.current[formKey] ||
        !isEmail.current.test(formRef.current[formKey]));
    const inValidBorder = () =>
      isNotValid() ? ' border-red-500' : `${disabled ? ' bg-gray-200' : ''}`;
    const errorNum = () =>
      !formRef.current[formKey]
        ? 1
        : !isEmail.current.test(formRef.current[formKey])
    ...
    return (
      <div className={`${styleClass}${isNotValid() ? ' mark-err' : ''}`}>
        ...
        {isNotValid() && (
          <div className="flex justify-start items-center text-red-500 text-xs mt-1">
            {errorNum() === 1 ? required : 'Invalid email address'}
          </div>
        )}
      </div>
    );
...
export const InputRadio = React.memo(
  ({
    ...
    const isNotValid = () =>
      required && formRef.current.isSubmit && !formRef.current[formKey];
    const inValidBorder = () =>
      isNotValid() ? 'border-red-500' : 'border-gray-400';
    const [, refresh] = useState(-1);
    const setFormVal = (e) => {
      formRef.current[formKey] = e.currentTarget.value;
      refresh(Date.now());
              ...
              <span className="ml-2 text-gray-600">
                {icons[i] && (
                  <FontAwesomeIcon
                    ...
                    style={{
                      color:
                        formRef.current[formKey] === r
                          ? colors[i] || 'rgb(75 85 99)'
                          : 'rgb(75 85 99)',
                    }}
                  />
                )}
    ...
    return (
      <div className={`${styleClass}${isNotValid() ? ' mark-err' : ''}`}>
        ...
        <div className="flex">
          ...
          >
            {formRef.current[code] === '-' ? (
              '-'
            ) : (
              <>
                <span className="mr-2">{formRef.current[code]}</span>
                <span>{formRef.current[code] === '+1' ? <Us /> : <In />}</span>
              </>
            )}
          </span>
          <input
...
export const InputDOB = React.memo(
  ({
    styleClass,
    formKey,
    formRef,
...
export const ForwardDatePicker = React.memo(
  ({
    styleClass,
    formKey,
    formRef,
...
export const PasswordCheck = React.memo(
  ({
    styleClass,
    formKey,
    formRef,
    ...
    passwordRLabel = 'Re-Password',
  }) => {
    const isNotValid = () =>
      formRef.current.isSubmit && !formRef.current[formKey];
    const inValidBorder = () => {
      return isNotValid()
        ? 'border-red-500'
        : formRef.current.isSubmit && feed >= 0 && feed < 2
        ? 'border-red-500'
        : 'border-gray-400';
    };
      ...
      <>
        <div
          className={`flex flex-col${
            isNotValid() || (formRef.current.isSubmit && feed >= 0 && feed < 2)
              ? ' mark-err'
              : ''
          }${formRef.current[formKey].length ? '' : ' mb-4'}`}
        >
          ...
          <div className="relative">
            ...
            />
            <div className="absolute inset-y-0 left-1 flex items-center px-2 pointer-events-none text-gray-600">
          ...
          {formRef.current[formKey].length > 0 && (
            <PasswordStrengthBar
        ...
        >
          ...
          <div className="relative">
            <input
              ...
              className={`border ${
                comparePass() ? 'border-red-500' : 'border-gray-400'
              } w-full py-2 pl-11 pr-10 rounded focus:border-dodge-b`}
...
export const GroupInput = React.memo(
  ({
    styleClass,
    formKey,
    formRef,
      ...
      if (mode === Mode.onlyAlphabets) {
        textRef.current.addEventListener('keypress', (event) => {
          const isValid =
            ...
            (event.charCode > 96 && event.charCode < 123); //(event.keyCode > 64 && event.keyCode < 91) || (event.keyCode > 96 && event.keyCode < 123);
...
export const GroupEmail = React.memo(
  ({
    styleClass,
    formKey,
    formRef,
...
export const CountryDropdown = React.memo(
  ({
    styleClass,
    passForm,
    valueKey,
