...
const ScheduleForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, scheduleAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      scheduleAddedList.current[recordIndex].documents.push({
    ...
    const saveSchedule = () => {
        ...
        : [
            {
              _modal: 'EventList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'schedule.id': arr.id },
              _data: { $set: { 'schedule.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...scheduleAddedList.current];
          newlist[recordIndex] = arr;
          scheduleAddedList.current = newlist;
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Schedule Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          scheduleAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeSchedule = () => {
      if (scheduleAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            scheduleAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { schedule: { id: formRef.current.id } } },
              },
            ];
            ...
            <form>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputSelect
                    ...
                    formKey="scheduleType"
                    ID="scheduleType"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Schedule Type"
                    placeholder="--Select--"
                    options={formList.scheduleType}
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="w-1/3 mr-5">
                  <ForwardDatePicker
                    ...
                    callback={() =>
                      dobCallback(formRef.current.scheduleDate, formRef.current)
                    }
                  />
                </div>
                <div className="w-1/3 mr-5 ">
                  {/* <InputSelect
                    ...
                    label="Day"
                    placeholder="--Select--"
                    options={formList.day}
                    required="Day is required"
                  /> */}
                  <div className="flex flex-col mb-4">
                    <label className="text-gray-600 required" htmlFor="day">
                      Day
                    </label>
                    <input
                      type="text"
                      readOnly={true}
                      value={formRef.current.day}
                ...
                <div className="w-1/3 mr-5">
                  <InputSelect
                    styleClass="flex flex-col mb-4"
                    formKey="scheduleTime"
                    ID="scheduleTime"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Schedule Time"
                    placeholder="--Select--"
                    options={formList.scheduleTime}
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.scheduleDescription
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.scheduleDescription}
