...
myLocalStorage.prototype.getLoginInfo = function () {
  return JSON.parse(localStorage.getItem(this.login) || '{}');
};
myLocalStorage.prototype.getUserId = function () {
  return JSON.parse(localStorage.getItem(this.login) || '{}')._id;
};
myLocalStorage.prototype.empty = function () {
  localStorage.clear();
};
myLocalStorage.prototype.getShortName = function () {
  if (this.getLoginInfo().accessType === 0) return 'AD';
  else
    return `${this.getLoginInfo().firstName.charAt(
      0
    )}${this.getLoginInfo().lastName.charAt(0)}`;
};
myLocalStorage.prototype.getFullName = function () {
  return `${this.getLoginInfo().firstName} ${this.getLoginInfo().lastName}`;
};
myLocalStorage.prototype.getEmail = function () {
...
myLocalStorage.prototype.getProfilePic = function () {
  return this.getLoginInfo().logo_path || '';
};
