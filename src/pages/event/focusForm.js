...
const FocusForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
      focusAddedList.current[recordIndex].documents.push({
    ...
    const saveFocus = () => {
        ...
        : [
            {
              _modal: 'EventList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'focus.id': arr.id },
              _data: { $set: { 'focus.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...focusAddedList.current];
          newlist[recordIndex] = arr;
          focusAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Focus Event Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Focus Event Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          focusAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeFocus = () => {
      if (focusAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            focusAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Focus Event Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { focus: { id: formRef.current.id } } },
              },
            ];
        ...
        <div className="p-5 border rounded shadow-md relative" ref={currentDom}>
          ...
          <div className="pt-5 pb-3">
                  ...
                  <InputSelect
                    ...
                    placeholder="--Select--"
                    options={formList.focusArea}
                    required="Our Focus Area is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
