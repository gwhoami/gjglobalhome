...
const CustcommentForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      custcommentAddedList.current[recordIndex].documents.push({
    ...
    const saveCustcomment = () => {
        ...
        : [
            {
              _modal: 'EventList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'custcomment.id': arr.id },
              _data: { $set: { 'custcomment.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...custcommentAddedList.current];
          newlist[recordIndex] = arr;
          custcommentAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Custcomment Event Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Customer-comments Event Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          custcommentAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeCustcomment = () => {
      if (custcommentAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            custcommentAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Customer-comments Event Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { custcomment: { id: formRef.current.id } } },
              },
            ];
          ...
          <div className="pt-5 pb-3">
                ...
                <div className="w-1/3 mr-5">
                  ...
                  <InputSelect
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Client/Cus/Public"
                    placeholder="--Select--"
                    options={formList.clientEvent}
                  ...
                  />
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                ...
                                <textarea
                                    className={`w-full rounded border ${!formRef.current.custDescription ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    value={formRef.current.custDescription}
                                    onChange={e => { formRef.current.custDescription = e.currentTarget.value; subRefresh(Date.now()); }}
                                    rows={4}
                                >
                                </textarea>
                            </div>
...
export default CustcommentForm;
