...
const GalleryForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
      galleryAddedList.current[recordIndex].documents.push({
    ...
    const saveGallery = () => {
        ...
        : [
            {
              _modal: 'EventList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'gallery.id': arr.id },
              _data: { $set: { 'gallery.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...galleryAddedList.current];
          newlist[recordIndex] = arr;
          galleryAddedList.current = newlist;
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Gallery Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          galleryAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeGallery = () => {
      if (galleryAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            galleryAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Gallery Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { gallery: { id: formRef.current.id } } },
              },
            ];
            ...
            <div className="flex w-full justify-start items-center relative">
              ...
              <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                ...
                                <textarea
                                    className={`w-full rounded border ${!formRef.current.galleryDescription ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    value={formRef.current.galleryDescription}
                                   placeholder='Description'
                    onChange={e => { formRef.current.galleryDescription = e.currentTarget.value; subRefresh(Date.now()); }}
                                    rows={4}
                                >
                                </textarea>
                            </div>
